...
public static class UrpglibReader
{
	...
	/// <param name="readPayloadIntoMemory">
	...
	public static async Task<UrpglibPackage> ReadAsync(string filePath, bool readPayloadIntoMemory = false)
	{
		...
		if (fileStream.Length < UrpglibConstants.MinFileSize)
		{
			...
			await fileStream.DisposeAsync().ConfigureAwait(false);
			throw new UrpglibFileSizeException(
				$"The file '{filePath}' is smaller than the minimum required size of {UrpglibConstants.MinFileSize} bytes.",
				new EndOfStreamException(
					$"Stream must be at least {UrpglibConstants.MinFileSize} bytes, was actually {length} bytes"));
		}
		...
		{
			await fileStream.DisposeAsync().ConfigureAwait(false);
			throw new UrpglibVersionMismatchException(
				$"File header version ({header.HeaderSchemaVersion}) is newer than supported version ({UrpglibConstants.CurrentHeaderSchemaVersion}). Please update the application.");
		}
		...
		if (header.ManifestSchemaVersion > UrpglibConstants.CurrentManifestSchemaVersion)
		{
			await fileStream.DisposeAsync().ConfigureAwait(false);
			throw new UrpglibVersionMismatchException(
				$"Manifest schema version ({header.ManifestSchemaVersion}) is newer than supported version ({UrpglibConstants.CurrentManifestSchemaVersion}). Please update the application.");
		}
		...
		{
			await fileStream.DisposeAsync().ConfigureAwait(false);
			throw new UrpglibFileFormatException(
				$"Manifest length ({header.ManifestLength}) is less than 1 byte. The file may be corrupted.");
		}
		...
		if (header.ManifestLength > fileStream.Length - fileStream.Position)
		{
			...
			await fileStream.DisposeAsync().ConfigureAwait(false);
			throw new UrpglibFileFormatException(
				$"Manifest length ({header.ManifestLength}) exceeds remaining file size ({remainingBytes}). The file may be corrupted.");
		}
		...
		try
		{
			manifest = JsonSerializer.Deserialize<PackageManifest>(manifestJsonBytes,
